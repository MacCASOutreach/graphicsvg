module GraphicSVG.Widget exposing
    ( icon
    , Msg, Model
    , init, subscriptions
    , update
...
import Svg
import Svg.Attributes exposing (clipPath, height, id, viewBox, width, x, y)
import Task
...
getContainerSize id =
    Task.attempt
        (\rvp ->
            case rvp of
                Ok vp ->
                    WidgetResize <| Just ( vp.viewport.width, vp.viewport.height )

                _ ->
                    WidgetResize <| Just ( 0, 0 )
        )
        (getViewportOf id)
...
viewCustom viewOptions model shapes =
    let
        positionWrapper toMsg ( x, y ) =
            toMsg <| convertCoords model.ww model.wh model.cw model.ch ( x, y )
    in
    Svg.svg
        ([ id model.id
         , viewBox
