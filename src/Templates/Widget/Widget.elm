...
initialModel =
    let
        ( wstate0, wcmd0 ) =
            Widget.init 50 25 "widget0"

        ( wstate1, wcmd1 ) =
            Widget.init 90 25 "widget1"
    in
    ( { count = 0
...
update msg model =
    case msg of
        WidgetOneMsg wMsg ->
            let
                ( newWState, wCmd ) =
                    Widget.update wMsg model.widgetOneState
            in
            ( { model
                | widgetOneState = newWState
              }
        ...
        WidgetTwoMsg wMsg ->
            let
                ( newWState, wCmd ) =
                    Widget.update wMsg model.widgetTwoState
            in
            ( { model
                | widgetTwoState = newWState
              }
