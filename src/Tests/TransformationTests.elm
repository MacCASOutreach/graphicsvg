module Main exposing (Model, Msg(..), checkIcon, clock, init, main, renderSuccesses, tests, title, update, view, xIcon)
...
update msg model =
    case msg of
        ...
        CorrectTest ->
            ( { model
                | successful = Dict.insert model.test True model.successful
        ...
        IncorrectTest ->
            ( { model
                | successful = Dict.insert model.test False model.successful
                , test = clamp 0 (Array.length tests) (model.test + 1)
              }
            , Cmd.none
            )
...
view model =
    ...
    collage 192
        ...
        (if model.test < Array.length tests then
            ...
            [ renderSuccesses model.successful ]
        )
